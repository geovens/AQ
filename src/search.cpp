		...
		std::vector<std::pair<int,int>>& serch_route, LGR& lgr_, Statistics& stat_)
{
	...
	if (need_rollout)
	{
		...
		// AQ-PS: value network is removed
		/*
		value_result = 0;
		if(pc->is_value_eval){
		...
		*/
		...
		if(use_rollout){
			...
			rollout_result = -PlayoutLGR(b, lgr_, komi) * win_bias;
		}
	}
	else{
		// a-2. 次のnodeに進む
						...
						bool is_errout, bool is_ponder)
{
	...
	else
	{
		...
		else
		{
			...
			if (pn->total_game_cnt - prev_game_cnt > 500) stat -= prev_stat;
...
void Tree::PrintChildInfo(int node_idx, std::ostream& ost){
	...
	for(int i=0;i<std::min((int)pn->child_cnt, 10);++i) {
		...
		else ost << "|" << std::setw(6) << std::fixed << rollout_rate * 100;
